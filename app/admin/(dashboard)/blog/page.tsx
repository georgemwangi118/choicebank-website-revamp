  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-[#0A0534]">Blog Posts</h1>
          <p className="text-sm text-gray-400 mt-1">{posts?.length ?? 0} total posts</p>
        </div>
        <Link
          href="/admin/blog/new"
          className="inline-flex items-center gap-2 bg-[#0A0534] text-white px-5 py-2.5 rounded-full text-sm font-semibold hover:bg-[#0A0534]/90 transition-colors"
        >
          <AddIcon /> New Post
        </Link>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        {!posts?.length ? (
          <div className="p-12 text-center text-gray-400">No posts yet. Create your first one.</div>
        ) : (
            ...
            <tbody className="divide-y divide-gray-50">
              {(posts as Post[]).map((post) => (
                <tr key={post.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-4 font-medium text-[#0A0534] max-w-xs">
