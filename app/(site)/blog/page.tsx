...
export default async function BlogPage() {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {!posts.length ? (
        <div className="py-32 text-center text-gray-400">No posts published yet. Check back soon.</div>
      ) : (
        <>
          {/* Featured post */}
                ...
                <div className="grid md:grid-cols-2 gap-0 rounded-3xl overflow-hidden bg-white shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                  <div className="relative h-72 md:h-auto">
                    {featured.cover_image && (
                      <Image src={featured.cover_image} alt={featured.title} fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover" />
                    )}
                    ...
                    <Link key={post.slug} href={`/blog/${post.slug}`} className="group block">
                      <div className="rounded-2xl overflow-hidden border border-gray-100 hover:shadow-md transition-shadow h-full flex flex-col">
                        <div className="relative h-52">
                          {post.cover_image && (
                            <Image src={post.cover_image} alt={post.title} fill sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" className="object-cover" />
                          )}
                          <div className="absolute inset-0 bg-[#0A0534]/20" />
                        ...
                        <div className="p-6 flex flex-col flex-1">
                          <h3 className="font-bold text-[#0A0534] text-lg leading-snug mb-3 group-hover:text-[#E8192C] transition-colors">{post.title}</h3>
